...
Remember that to-do list app you built in the first chapter? It probably worked fine for a prototype. But what happens when you come back a week later and want to add user authentication? Or when you realize you need to handle edge cases like duplicate tasks or data persistence?

You probably can't remember exactly what you asked Claude to do. The AI certainly doesn't remember. And if you ask it to add features, there's a good chance it will rewrite things in ways that break what was working before.

This is the Achilles' heel of pure vibe coding: **it doesn't scale beyond quick prototypes**.

As one developer put it on a popular tech forum: "Vibe coding got me 80% of the way there in 20% of the time. Then I spent 80% of my time fixing the last 20%."

When you're building something that needs to work reliably, you run into problems:

- **Context amnesia**: The AI forgets previous decisions across sessions
- **Inconsistent implementations**: The same request yields wildly different code
- **Debugging nightmares**: You can't trace why something was built a certain way
- **Team chaos**: Other people can't see the reasoning behind your code
- **Breaking changes**: Modifications break things because requirements were never documented

This is where **Spec-Driven Development** comes in.

## What is Spec-Driven Development?

Spec-Driven Development (SDD) is exactly what it sounds like: you write specifications before you write code. But unlike the 100-page requirements documents of yesteryear that nobody read, modern SDD is lightweight, structured, and designed specifically for working with AI coding agents.

At its core, SDD means creating three types of documents:
...
Think of it as giving your AI assistant a detailed brief instead of vague instructions. The difference between saying "build me a house" versus handing over blueprints with specific dimensions, materials, and electrical layouts.

Here's what Sean Grove from OpenAI said at the AI Engineer conference in 2025:
...
## Web Application Building Blocks: A PM's Guide

Before we dive into spec-driven development, you need to understand what you're actually specifying. When you write requirements and design documents for web applications, you'll be working with four fundamental building blocks. Think of these as the main characters in every web app story:

### Frontend (The User Interface)

The **frontend** is everything users see and interact with in their browser. It's the buttons, forms, text, images, and animations. When you click "Add to Cart" on a shopping site, that button and the visual feedback you get - that's frontend.

**Technologies you'll encounter:**
- **HTML** - The structure (like the skeleton of a webpage)
- **CSS** - The styling (colors, fonts, layouts)
- **JavaScript** - The interactivity (what happens when you click things)
- **Frameworks** - React, Vue, Svelte (tools that make building complex interfaces easier)

**Real example:** In your to-do list app, the input box where you type tasks, the checkbox you click to mark tasks complete, and the list display - all frontend.
...
AND show a success animation
```

### Backend (The Business Logic)

The **backend** is the server-side code that runs behind the scenes. It handles the "business logic" - the rules and operations that make your app work. When you submit a login form, the backend checks if your password is correct. When you add a to-do item, the backend validates it and saves it.
...
**Real example:** When you mark a to-do item complete, the backend receives that request, validates that you own that task, updates the status, and confirms the change.

**When specifying backend:**
```markdown
...
The **database** is where your application stores data permanently. Think of it as your app's long-term memory. When you close your browser and come back later, how does the app remember your to-do items? The database.

**Technologies you'll encounter:**
- **SQL databases** - PostgreSQL, MySQL (organized like spreadsheets with tables and rows)
- **NoSQL databases** - MongoDB, Firebase (organized like nested documents)
- **ORMs** - Tools that let you interact with databases using normal code instead of database query language

**Real example:** Your to-do list items are stored in a database table that might look like:
...
**API** stands for Application Programming Interface, but think of it as the messenger system between frontend and backend. When your frontend needs data or wants to save something, it sends a request to an API endpoint. The backend processes it and sends a response back.

**Common API patterns:**
- **REST** - The most common style, uses HTTP methods (GET, POST, PUT, DELETE)
- **GraphQL** - Lets frontend request exactly the data it needs
- **Endpoints** - Specific URLs for specific operations (like `/api/tasks` or `/api/users/login`)
...
Here's how all four pieces work together when a user adds a to-do item:
...
sequenceDiagram
    ...
    User->>Frontend: Types "Buy groceries" and clicks Add
...
**This is called the "full stack"** - frontend, backend, database, and the APIs connecting them. When you become a **Full-Stack Product Builder**, you're able to specify requirements and design across all these layers.
...
**Frontend:**
- Registration form at /signup with email and password fields
...
See the difference? The good spec thinks through the entire stack. This is what Claude Code (or any AI coding agent) needs to generate quality code.
...
Spec-driven development isn't about abandoning the speed and creativity of vibe coding. It's about adding structure where it matters.

Think of it this way:
- **Vibe coding** is like sketching with a pencil - fast, exploratory, low commitment
- **SDD** is like working with blueprints - structured, reviewable, production-ready

You need both.

Use vibe coding to explore and prototype rapidly. Use SDD when you're ready to build something that lasts.

The developers building AI-native companies at scale aren't choosing between creativity and structure. They're using AI to move faster through both phases - vibe coding to iterate on ideas quickly, then SDD to ship production-quality features reliably.

As you continue through this workshop, we'll use SDD principles to build increasingly sophisticated features. The investment you make in learning this process now will pay dividends when you're building real products.

The future belongs to PMs who can both vibe code a prototype in an afternoon and spec-drive a production feature the next day.
